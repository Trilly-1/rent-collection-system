...
import { Mail, Lock, Eye, EyeOff, Sparkles, Receipt, User } from 'lucide-react';
...
export default function TenantLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await login('tenant', form.identifier, form.password);
    ...
    >
      <div className="rounded-3xl border border-line-200 bg-white p-8 shadow-2xl sm:p-10 lg:p-12">
        <div className="mb-8 flex items-center justify-center gap-2.5 text-gold-600">
          ...
          <p className="text-sm font-semibold uppercase tracking-wider">Tenant Portal</p>
        </div>
        ...
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="font-display text-4xl font-bold text-ink-950">Tenant Sign In</h1>
          <p className="mt-3 text-lg text-ink-700">
            Access your rent, payments, and lease details in one secure place.
          </p>
        </div>
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <Input
            ...
            type="text"
            icon={Mail}
            placeholder="0782 000 000 or email"
        ...
        <div className="mt-6 flex items-center justify-between text-sm">
          <Link to="/forgot-password?role=tenant" className="font-semibold text-ink-600 hover:text-ink-900 transition-colors">
            Forgot password?
          </Link>
          <Link to="/tenant-register" className="font-semibold text-gold-600 hover:text-gold-700 transition-colors">
            Create Account
          </Link>
        </div>
      </div>
